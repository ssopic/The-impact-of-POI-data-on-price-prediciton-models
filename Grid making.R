...
names(KingCounty)
BusinessData <- read.csv("C:/Users/silvi/OneDrive/Desktop/THESIS/Dataset Preprocessing/Business_Data_Total.csv")
names(BusinessData)
...
#Full value
#length()
dataframe <- data.frame(matrix(ncol=21613,nrow=4436))

for(d in 1:KingCounty$id){
...
#Full length
#
for(i in 1:length(BusinessData$id)){
...
print(paste0("Progress=",d," out of ",length(KingCounty$id)))
##This should show the progress bar better
}
write.csv(dataframe, "distance.csv")      
